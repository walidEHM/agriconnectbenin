<?php
session_start();
if (!isset($_SESSION['marche_id']) || !isset($_GET['conversation_id'])) {
...
$conn->prepare("UPDATE messages SET lu = 1 WHERE conversation_id = ? AND destinataire_id = ? AND destinataire_type = 'marche'")
    ->execute([$conversation_id, $marche_id]);
// Récupérer les messages
$stmt = $conn->prepare("SELECT * FROM messages WHERE conversation_id = ? ORDER BY date_envoi ASC");
$stmt->execute([$conversation_id]);
$messages = $stmt->fetchAll(PDO::FETCH_ASSOC);
foreach ($messages as $msg) {
    $isMe = $msg['expediteur_type'] === 'marche' && $msg['expediteur_id'] == $marche_id;
