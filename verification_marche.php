...
if (isset($_GET['token']) && isset($_GET['id'])) {
    ...
    try {
        $sql = "SELECT id, date_inscription FROM marches 
                WHERE id = :id
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $date_inscription = strtotime($marche['date_inscription']);
            ...
            if ($difference_seconds < 86400) {
                ...
                $stmt = $conn->prepare("UPDATE marches 
                                      SET compte_active = 1, 
...
<body>
    <div class="container">
        ...
        <h1>Compte Marché Activé!</h1>
        <p>Votre compte marché a été activé avec succès. Vous pouvez maintenant accéder à votre espace et commencer à connecter avec les producteurs agricoles du Bénin.</p>
        <a href='identification_marche.php#showLoginForm' class='btn'>
...
<body>
    <div class="container">
        ...
        <div class="btn-group">
            <a href='identification_marche.php#showRegisterForm' class='btn btn-primary'>
...
<?php
                ...
                $stmt = $conn->prepare("DELETE FROM marches WHERE id = :id AND compte_active = 0");
                $stmt->bindParam(':id', $id);
                $stmt->execute();
            }
        } else {
